...
case "$2" in
   ...
   -view-the-html-docs | -vthtmld) echo "$2 selected"
      ...
      ;;
   *) echo "No such command $2 supported for run_doxygen_commands.sh currently"
      ;;
esac
...
then
   ...
   echo "Usage : Argument 2: $2 needs to be the doxygen command selected i.e.
...
MUTEX_LOCK="$BUILD_DIR/.doxygen_lock"
...
RETURN_STATUS=$(function_run_doxygen_command $PROJECT_DIRECTORY $2)
